...
class Plans(Base):
    ...
    created_by_user_id: Mapped[sa.UUID] = mapped_column(sa.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, unique=False)

    created_by_user = relationship("Users", back_populates="plans", foreign_keys=[created_by_user_id])
    questions = relationship("Questions", back_populates="plan", cascade="all, delete-orphan", passive_deletes=True)
    interviews = relationship("Interviews", back_populates="plan", cascade="all, delete-orphan", passive_deletes=True)
